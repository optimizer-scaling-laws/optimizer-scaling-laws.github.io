...
Realized capacity can differ under one architecture because learning is shaped by inductive bias, and architecture is only part of it. Since data rarely specifies a unique model, a learner needs biases that privilege some solutions over others (<a href="https://arxiv.org/abs/2304.05366" target="_blank" rel="noopener noreferrer">Goldblum et al., 2024</a>). Architecture supplies a hard bias: causal masking, residual topology, attention structure, parameter sharing, and routing restrict the support of possible computation. The optimizer supplies a softer one — it leaves the formal hypothesis space largely intact but changes which solutions are reachable, stable, and likely under a given training budget, in line with the view that modern overparameterized learning keeps a flexible hypothesis space and imposes soft preferences over data-consistent solutions (<a href="https://arxiv.org/abs/2503.02113" target="_blank" rel="noopener noreferrer">Wilson, 2025</a>). What this post adds is a place to measure that soft preference: realized spectral capacity is one axis on which it becomes visible — and on which optimizers separate even where their loss does not.

This soft bias has the most room to act where data constrains the solution weakly — the rare-token regimes, where many directions are compatible with nearly the same loss. It also means the effective computational graph is not fixed by architecture alone. The formal Transformer graph defines which paths can carry signal; which ones do — which heads, FFN directions, residual-stream features, and layer compositions become load-bearing — is shaped by the optimizer. The transformer-circuits view decomposes a model into residual-stream paths read and written by attention heads (<a href="https://transformer-circuits.pub/2021/framework/index.html" target="_blank" rel="noopener noreferrer">Elhage et al., 2021</a>); our results suggest which of those paths are active is conditional on the architecture–optimizer pair, not the architecture alone.

#### Forward-looking implication: realized capacity may matter for plasticity
{: #forward-looking-implication-realized-capacity-may-matter-for-plasticity}

Continual learning raises a forward-looking version of the same question. Forgetting asks whether a model keeps the past; plasticity asks whether it can still learn the future. This is a conjecture rather than something the spectral measurements establish: the paper measures realized capacity now, while future learnability depends on whether realized or weakly used directions remain available for later adaptation. What makes it worth stating is that the loss-blindness documented here for current capacity reappears for future adaptability. Han et al. find that more pretraining weight decay produces more adaptable base models — larger downstream gains after fine-tuning — even when a lower validation loss would have selected a different checkpoint (<a href="https://arxiv.org/abs/2602.11137" target="_blank" rel="noopener noreferrer">Han et al., 2026</a>): the same decoupling of loss from the property that matters, moved one stage downstream. The most direct evidence sits at LLM scale: GPT-style models trained on long multilingual text streams measurably lose the ability to absorb a held-out language, and they do so even under stationary pretraining, where validation loss keeps falling while adaptability erodes (<a href="https://arxiv.org/abs/2606.24752" target="_blank" rel="noopener noreferrer">Hernandez-Garcia et al., 2026</a>). A falling loss curve and a decaying ability to learn are not mutually exclusive; here they coincide.

If realized capacity does bound later learnability, the shared currency is the set of usable directions — the effective rank this post already tracks. The plasticity literature converges on it: across reinforcement learning, vision, and language, loss of plasticity surfaces as a collapse of that rank — dormant units, falling effective feature rank, a degenerating curvature spectrum (<a href="https://arxiv.org/abs/2303.07507" target="_blank" rel="noopener noreferrer">Abbas et al., 2023</a>; <a href="https://www.nature.com/articles/s41586-024-07711-7" target="_blank" rel="noopener noreferrer">Dohare et al., 2024</a>; <a href="https://arxiv.org/abs/2402.18762" target="_blank" rel="noopener noreferrer">Lyle et al., 2024</a>; <a href="https://arxiv.org/abs/2509.22335" target="_blank" rel="noopener noreferrer">He et al., 2025</a>) — and at LLM scale the same picture shows up directly, as dormant FFN units and attention heads that collapse or go lazy while a model loses its grip on new data. What differs across these results is not the symptom but the lever pushing on the rank. In Wilson's hard/soft bias language, architecture provides the harder structural constraint, while optimization provides the softer dynamical bias that determines which capacity is actually realized during training.

**Two levers, one currency.** Realized capacity is the count of usable representational directions; architecture and the optimizer push on it from opposite sides.
...
The bottom of that table is where the two sides turn concrete. On the dynamical side, PAPO restores plasticity by adding a fraction of the gradient's polar factor to each update, uniformly lifting the near-zero singular values that stability-preserving methods drive toward zero (<a href="https://openreview.net/forum?id=b7P2WegaBY" target="_blank" rel="noopener noreferrer">Zheng et al., 2026</a>) — and that polar factor is exactly the orthogonalization that separates Muon-style updates from AdamW, so the very operation behind the spectral differences measured here reappears as a lever for preserving plasticity. On the structural side, width and depth already split the trade-off — wider forgets less, deeper stays more plastic (<a href="https://arxiv.org/abs/2110.11526" target="_blank" rel="noopener noreferrer">Mirzadeh et al., 2022</a>; <a href="https://arxiv.org/abs/2506.03951" target="_blank" rel="noopener noreferrer">Lu et al., 2025</a>) — and interpolation layers hold open a non-collapsing rank pathway with no optimizer-level change at all (<a href="https://openreview.net/forum?id=pAhGjPOlwy" target="_blank" rel="noopener noreferrer">Koeppe et al., 2026</a>). Scale sits across both columns and resolves neither: more parameters postpone the onset of plasticity loss, but only sublinearly — delaying the collapse without preventing it — even as they let a model hold rare-task structure longer by reducing interference (<a href="https://arxiv.org/abs/2605.29548" target="_blank" rel="noopener noreferrer">Huang et al., 2026</a>).

The continual-learning reading is not that architecture stops mattering. It is that capacity must be realized during pretraining and preserved in a form that stays available for later learning — which is, in the end, what a formal account of plasticity measures: how much an agent can still be shaped by what it observes (<a href="https://arxiv.org/abs/2505.10361" target="_blank" rel="noopener noreferrer">Abel et al., 2025</a>). Scale postpones the loss of that form but does not secure it; which forms survive depends on architecture, optimizer, regularization, and training dynamics together. The design object, once again, is the architecture–optimizer pair.
